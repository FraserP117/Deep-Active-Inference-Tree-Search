...
class Agent():
    ...
    def compute_value_net_loss(
        ...
        # print("\tINSIDE compute_value_net_loss")
    ...
    def compute_VFE(self, obs_batch_t1, pred_error_batch_t0t1):
        ...
        EFEs_batch_t1 = self.value_net(obs_batch_t1).detach() # shape: [64, 2]
        ...
        # print(f"boltzmann_EFEs_batch_t1.shape: {boltzmann_EFEs_batch_t1.shape}")
        # print(f"self.batch_size: {self.batch_size}")
        # print(f"policy_batch_t1.shape: {policy_batch_t1.shape}\n\n")

        # EFEs_batch_t1.shape: torch.Size([64, 2])
        # boltzmann_EFEs_batch_t1.shape: torch.Size([64, 2])
        # self.batch_size: 64
        # policy_batch_t1.shape: torch.Size([64, 2])
    ...
    def compute_EFE(self, obs_batch_t2, pred_error_batch_t0t1):
        ...
        with torch.no_grad():
            ...
            # Determine the action distribution for time t2:
            policy_batch_t2 = self.policy_net(obs_batch_t2)  # Assuming it is normalized

            # Calculate the prior distribution over observations (p(o_tau))
            prior_obs_dist = 1.0 / len(policy_batch_t2[0])
            prior_obs = torch.full_like(policy_batch_t2, prior_obs_dist)

            # print(f"\n\npred_error_batch_t0t1.shape: {pred_error_batch_t0t1.shape}")
            # print(f"policy_batch_t2.T.shape: {policy_batch_t2.T.shape}")
            # print(f"policy_batch_t2[:, :, None].shape: {policy_batch_t2[:, :, None].shape}\n\n")

            # policy_batch_t2.T = batch of lists, each containing the probability of each action in the action space

            # Calculate q(o_tau | pi) by marginalizing over state s_tau
            q_obs_given_policy = torch.matmul(pred_error_batch_t0t1, policy_batch_t2.T)

            # Calculate the joint distribution p(o_tau, s_tau)
            joint_obs_state_dist = pred_error_batch_t0t1 * policy_batch_t2[:, :, None]

            # Calculate p(o_tau | s_tau) by summing over the states
            p_obs_given_state = joint_obs_state_dist.sum(dim=1)

            # Calculate the KL divergence term
            # kl_divergence = F.kl_div(q_obs_given_policy.log(), prior_obs, reduction='sum')
            kl_divergence = F.kl_div(q_obs_given_policy, prior_obs, reduction='sum')

            # Calculate the entropy term (expectation over q(o_tau | pi))
            entropy_term = torch.mean(compute_entropy(p_obs_given_state)) # expectation under 𝑄̃ = 𝑄(𝑜_𝜏 , 𝑠_𝜏 |𝜋) ≜ 𝑃(𝑜_𝜏 |𝑠𝜏_ )𝑄(𝑠_𝜏 |𝜋)

            # Compute the EFE as per the correct definition
            EFE_estimate_batch = -kl_divergence - entropy_term

        return EFE_estimate_batch
