...
class MVGaussianModel(nn.Module):
    ...
    def __init__(self, n_inputs, n_outputs, n_hidden, lr=1e-3, device='cpu', softmax = False):
    ...
    def forward(self, x):
        ...
        if self.softmax:
            a = F.Softmax(x, dim = -1).clamp(min = 1e-9, max = 1 - 1e-9)
...
class Agent():
    ...
    def __init__(self, argv):
        ...
        # Initialize the proleptic networks - efe value and policy networks only used for bootstrapping phase of the algorithm
        self.policy_net = MVGaussianModel(self.obs_size, self.n_actions, self.n_hidden_pol, lr=self.lr_pol, softmax=True, device=self.device)
        self.efe_value_net = MVGaussianModel(self.obs_size, self.n_actions, self.n_hidden_val, lr=self.lr_val, device=self.device) # input: state_t, output: EFE for all actions in action space

        # Initialize the model networks:
        ...
        if self.load_network: # If true: load the networks given paths
            ...
            # p_xi
            self.generative_observation_net.load_state_dict(torch.load(self.network_load_path.format("gen_obs")))
            ...
            # q_pi
            self.policy_net.load_state_dict(torch.load(self.network_load_path.format("var_pol")))
            self.policy_net.eval()

            # f_psi - efe value net
            self.efe_value_net.load_state_dict(torch.load(self.network_load_path.format("efe_val")))
    ...
    def get_mini_batches(self):
        ...
        # At time t0 predict the state belief at time t1:
        # X = torch.cat((obs_batch_t0, action_batch_t0.float()), dim=1)
        # pred_batch_t0t1 = self.variational_transition_net(X)
        X = torch.cat((obs_batch_t0, action_batch_t0.float()), dim=1)
        pred_state_batch_belief_mean_t0t1, pred_state_batch_belief_var_t0t1 = self.variational_transition_net(X)

        # Determine the prediction error wrt time t0-t1:
        # pred_error_batch_t0t1 = torch.mean(F.mse_loss(
        #         pred_batch_t0t1, obs_batch_t1, reduction='none'), dim=1
        # ).unsqueeze(1)

        # predict the observation belief at time t1
        pred_obs_batch_belief_mean_t1, pred_obs_batch_belief_var_t1 = self.generative_observation_net(obs_batch_t1)

        # Determine the state-belief prediction error wrt time t0-t1:
        # pred_error_batch_t0t1 = torch.mean(F.mse_loss(
        #         pred_batch_t0t1, obs_batch_t1, reduction='none'), dim=1
        # ).unsqueeze(1)

        pred_error_batch_t0t1 = self.kl_divergence_diag_cov_gaussian(
            pred_state_batch_belief_mean_t0t1, pred_state_batch_belief_var_t0t1,
            pred_obs_batch_belief_mean_t1,pred_obs_batch_belief_var_t1
        ).unsqueeze(1)
    ...
    def compute_value_net_loss(
        ...
        with torch.no_grad():
            ...
            policy_batch_t2 = self.policy_net(obs_batch_t2) # better be normalised
    ...
    def compute_VFE(self, obs_batch_t1, pred_error_batch_t0t1):
        ...
        EFEs_batch_t1 = self.efe_value_net(obs_batch_t1).detach() # shape: [64, 2]

        # print(f"\n\nEFEs_batch_t1.shape: {EFEs_batch_t1.shape}")
        ...
        # Not this????????
        # energy_batch = -(policy_batch_t1 * torch.log(policy_batch_t1 / boltzmann_EFEs_batch_t1)).sum(-1).view(self.batch_size, 1) 
    ...
    def learn(self):
        ...
        # efe_computation = self.compute_EFE(obs_batch_t2, pred_error_batch_t0t1)
        # print(f"\n\nefe_computation: {efe_computation}\n\n")
        ...
        self.efe_value_net_loss.optimizer.zero_grad()
        self.generative_observation_net.optimizer.zero_grad()
        ...
        self.efe_value_net_loss.optimizer.step()
        self.generative_observation_net.optimizer.step()
        self.variational_transition_net.optimizer.step()
