...
class Agent():
    ...
    def do_aif_tree_search(self):
        # t = 0
        # while halting conditions not satisfied:
        #     x_t = update expected state belief using s_t, o_t, a_{t-1}, A, B, D
        #     create a node v(x_t, u_{t-1})
        #     while delta ** tau < epsilon:
        #         v_t = self.TreePolicy(v, B)
        #         G_delta = self.Eval(v_tau, A, B, C, delta)
        #         self.PathIntegration(v_tau, G_delta)
    ...
    def TreePolicy(self, v):
        ...
        #         return self.Expansion(v, B)
        #     else:
        #         v = self.VariationalInference(v)
        # return v_tau = v
        pass

    def Expansion(self, v, B):
        # randomly draw an unused action u_prime on v
        # for parent v, generate new child v_prime(x_prime, u_prime) with x_prime = B(u_prime) * x
        # return v_prime
        pass

    def VariationalInference(self, v):
        # build the distribution E via the probability mas function sqrt((2*ln(N(v)) / (N(v_prime)))
        # u_prime = Boltzmann(k_p * ln(E) = gamma * G_Delta(v_prime))
        # return v_prime(u_prime)
        pass

    def Eval(v_tau, A, B, C, delta):
        # evaluate EFE: G(*, v_tau) through A, B, C
        # return G_delta = delta ** tau * G(*, v_tau)
        pass

    def PathIntegration(self, v_tau, G_delta):
        # while v_tau not v:
        #     N(v_tau) += 1
    ...
    def train(self):
        ...
        for ith_episode in range(self.n_episodes):
            ...
            while not done:
                ...
                self.learn() # seems to be getting stuck here after a while 
