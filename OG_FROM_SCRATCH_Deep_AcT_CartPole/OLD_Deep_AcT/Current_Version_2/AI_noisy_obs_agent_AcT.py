...
def draw_tree(root_node, selected_action):
    ...
    plt.figure()  
    plt.title(f"1-ply AcT Planning Tree: selected action = {selected_action}")
...
class Node:
    ...
    def __str__(self):
        ...
        return (
            ...
            f"predictive_EFE = {str(self.predictive_EFE.item())},\n"
        )
...
class Agent():
    ...
    def __init__(self, argv):
        ...
        self.generative_observation_net = MVGaussianModel(self.obs_size, self.obs_size, self.n_hidden_gen_obs, lr=self.lr_gen_obs, device=self.device, model = 'gen_obs')
        self.variational_transition_net = MVGaussianModel(1, self.obs_size, self.n_hidden_var_trans, lr=self.lr_var_trans, device=self.device, model = 'var_trans')
        # self.variational_transition_net = MVGaussianModel(2*self.obs_size+1, self.obs_size, self.n_hidden_var_trans, lr=self.lr_var_trans, device=self.device, model = 'var_trans') # inputs: s_{t-1}, a_{t-1}, o_t
    ...
    def set_parameters(self, argv):
        ...
        default_parameters = {
            ...
            'env':'CartPole-v1', 'n_episodes':200, # OG VERSION FOR CARTPOLE-V1
    ...
    # def kl_divergence_diag_cov_gaussian(self, mu1, Sigma1, mu2, Sigma2, D): 
    #     '''
    #     Returns the average KL divergence for batched inputs. 
    #     In case of scaler inputs, simply returns the KL divergence for these two distributions
    #     '''
    ...
    #         Sigma1_i = torch.diag(Sigma1[i])
    #         Sigma2_i = torch.diag(Sigma2[i])

    #         Sigma1_inv_i = torch.inverse(Sigma1_i)
    #         Sigma2_inv_i = torch.inverse(Sigma2_i)

    #         trace_term_i = torch.trace(torch.matmul(Sigma2_inv_i, Sigma1_inv_i))

    #         mu_diff_i = mu2[i] - mu1[i]

    #         quad_term_i = torch.matmul(
    #             torch.matmul(
    #                 mu_diff_i.unsqueeze(0), 
    #                 Sigma1_inv_i
    #             ),
    #             mu_diff_i.unsqueeze(1)
    #         )

    #         det_Sigma1_i = torch.det(Sigma1_i)
    #         det_Sigma2_i = torch.det(Sigma2_i)

    #         log_term_i = torch.log(det_Sigma2_i / det_Sigma1_i)

    #         kl_div_i = 0.5 * (trace_term_i + quad_term_i - D + log_term_i)

    #         avg_kl_div += kl_div_i

    #         print(f"\n\nSigma1_i:\n{Sigma1_i}")
    #         print(f"Sigma2_i:\n{Sigma2_i}")
    #         print(f"Sigma1_inv_i:\n{Sigma1_inv_i}")
    #         print(f"Sigma2_inv_i:\n{Sigma2_inv_i}")
    #         print(f"trace_term_i: {trace_term_i}")
    #         print(f"mu_diff_i: {mu_diff_i}")
    #         print(f"quad_term_i: {quad_term_i}")
    #         print(f"det_Sigma1_i: {det_Sigma1_i}")
    #         print(f"det_Sigma2_i: {det_Sigma2_i}")
    #         print(f"log_term_i: {log_term_i}")
    #         print(f"kl_div_i: {kl_div_i}\n\n")

    #         breakpoint()

    #     final_avg_kl_div = avg_kl_div / batch_size

    #     return final_avg_kl_div
    ...
    def diag_cov_gaussian_log_li(self, predicted_cur_obs_mean, predicted_cur_obs_var, observation_samples, D):
        ...
        avg_ll = avg_ll / len(observation_samples)
    ...
    #     # print(f"\nobservation_samples:\n{observation_samples}\n")
    #     # print(f"predicted_cur_obs_mean:\n{predicted_cur_obs_mean}\n")
    #     # print(f"predicted_cur_obs_var:\n{predicted_cur_obs_var}\n")

    #     # Calculate dimensionality (d) from the length of predicted_cur_obs_var
    #     d = len(predicted_cur_obs_var)

    #     # Create a diagonal covariance matrix Σ
    #     Σ = torch.diag(predicted_cur_obs_var)
    ...
    #     observation_samples_t = observation_samples.unsqueeze(1)  # Transpose by adding an extra dimension
    #     observation_samples_minus_predicted_cur_obs_mean = observation_samples_t - predicted_cur_obs_mean

    #     # Calculate the quadratic term
    #     quad_term = 0.5 * torch.matmul(torch.matmul(observation_samples_minus_predicted_cur_obs_mean.transpose(0, 1), Σ_inv), observation_samples_minus_predicted_cur_obs_mean)
    ...
    #     return S_observation_samples_scalar.item()  # Convert the result to a Python float
    ...
    #         # Create a diagonal covariance matrix Σ
    #         Σ_i = torch.diag(predicted_cur_obs_var[i])

    #         # Calculate the inverse of Σ
    #         Σ_i_inv = torch.inverse(Σ_i)

    #         # Transpose x to make the matrix multiplication valid
    #         observation_samples_t = observation_samples[i].unsqueeze(1)  # Transpose by adding an extra dimension
    #         observation_samples_minus_predicted_cur_obs_mean = observation_samples_t - predicted_cur_obs_mean[i]

    #         print(f"\n\nobservation_samples[i]:\n{observation_samples[i]}\n")
    #         print(f"observation_samples_t:\n{observation_samples_t}\n")
    #         print(f"predicted_cur_obs_mean[i]:\n{predicted_cur_obs_mean[i]}\n")
    #         print(f"observation_samples_minus_predicted_cur_obs_mean:\n{observation_samples_minus_predicted_cur_obs_mean}\n\n")

    #         breakpoint()

    #         # print(f"\n\nobservation_samples_minus_predicted_cur_obs_mean.transpose(0, 1):\n{observation_samples_minus_predicted_cur_obs_mean.transpose(0, 1)}\n")
    #         # print(f"Σ_i_inv:\n{Σ_i_inv}\n")
    #         # print(f"observation_samples_minus_predicted_cur_obs_mean:\n{observation_samples_minus_predicted_cur_obs_mean}\n")

    #         # Calculate the quadratic term
    #         quad_term = 0.5 * torch.matmul(
    #             torch.matmul(
    #                 observation_samples_minus_predicted_cur_obs_mean.transpose(0, 1), 
    #                 Σ_i_inv
    #             ), 
    ...
    #         # Calculate the log determinant term
    #         log_det_term = 0.5 * torch.log(torch.det(Σ_i))
    ...
    #         # print(f"\n\nquad_term:\n{quad_term}\n")
    #         # print(f"log_det_term:\n{log_det_term}\n")
    #         # print(f"constant_term:\n{constant_term}\n")
    #         # print(f"S_observation_samples:\n{S_observation_samples}\n\n")
    ...
    #     return avg_neg_log_li/len(observation_samples)  
    ...
    def negative_log_likelihood(self, observation_samples, predicted_cur_obs_mean, predicted_cur_obs_var):
        ...
        better to use average over predicted_cur_obs_mean and average over predicted_cur_obs_var???????????????????????????????????????????????????????
        '''
        ...
        for i in range(len(observation_samples)):
            ...
            # print(f"Σ_i:\n{Σ_i}\n")
            # print(f"Σ_i_inv:\n{Σ_i_inv}\n")
            # print(f"observation_samples_minus_predicted_cur_obs_mean.unsqueeze(1):\n{observation_samples_minus_predicted_cur_obs_mean.unsqueeze(1)}\n")
            # print(f"quad_term:\n{quad_term}\n")
            # print(f"log_det_term: {log_det_term}\n")
            # print(f"constant_term: {constant_term}\n")
            # print(f"S_observation_samples:\n{S_observation_samples}\n")
            # print(f"S_observation_samples_scalar: {S_observation_samples_scalar}\n\n")
            # breakpoint()
    ...
    #         # Calculate the quadratic term
    #         quad_term = 0.5 * torch.matmul(torch.matmul(observation_samples_minus_predicted_cur_obs_mean.transpose(0, 1), Σ_i_inv), observation_samples_minus_predicted_cur_obs_mean)
    ...
    # def diag_cov_gaussian_log_evidence(predicted_cur_obs_mean, predicted_cur_obs_var, observation_samples, D):

    #     log_evidences = []

    #     for i in range(len(predicted_cur_obs_mean)):
    #         log_var = torch.log(predicted_cur_obs_var[i])
    #         log_likelihood = -0.5 * (
    #             log_var + ((observation_samples[i] - predicted_cur_obs_mean[i]) ** 2) / predicted_cur_obs_var[i] + D * np.log(2 * np.pi)
    #         )
    #         log_evidence_i = log_var + log_likelihood  # Log evidence for a single observation
    #         log_evidences.append(log_evidence_i.item())

    #     avg_log_evidence = np.mean(log_evidences)  # Average log evidence

    #     return avg_log_evidence

    # def diag_cov_gaussian_log_evidence(predicted_cur_obs_mean, predicted_cur_obs_var, observation_samples, D):

    #     # Create the corresponding diagonal multivariate Gaussian distribution/s
    #     diagonal_gaussian = torch.distributions.MultivariateNormal(
    #         predicted_cur_obs_mean, 
    #         covariance_matrix = torch.diag(predicted_cur_obs_var)
    #     )

    #     # Calculate the log likelihood/s for each data point
    #     log_likelihoods = diagonal_gaussian.log_prob(observation_samples)
    ...
    # def diag_cov_gaussian_log_evidence(self, predicted_cur_obs_mean, predicted_cur_obs_var, observation_samples, D):

    #     # print(f"\n\npredicted_cur_obs_mean:\n{predicted_cur_obs_mean}\n")
    #     # print(f"predicted_cur_obs_var:\n{predicted_cur_obs_var}\n")
    #     # print(f"observation_samples:\n{observation_samples}\n\n")
    ...
    def diag_cov_gaussian_log_evidence(self, predicted_cur_obs_mean, predicted_cur_obs_var, observation_samples, D):
        ...
        for i in range(len(observation_samples)):
            # Extract the mean, variance, and observation for the current vector in the batch
            mean_i = predicted_cur_obs_mean[i]
            var_i = predicted_cur_obs_var[i]
            obs_i = observation_samples[i]

            # Create a diagonal covariance matrix for the current vector
            covariance_matrix_i = torch.diag(var_i)

            # Create the corresponding diagonal multivariate Gaussian distribution
            diagonal_gaussian = torch.distributions.MultivariateNormal(
                mean_i,
                covariance_matrix = covariance_matrix_i
            )

            # Calculate the log likelihood for the current vector
            log_likelihood_i = diagonal_gaussian.log_prob(obs_i)

            # Compute the log evidence for the current vector
            log_evidence_i = log_likelihood_i.sum()

            avg_log_evidence.append(log_evidence_i)

        # Calculate the average log evidence for the entire batch
        avg_log_evidence = torch.mean(torch.stack(avg_log_evidence))
    ...
    #     # cast action_batch_t0 to right dtype
    #     action_batch_t0 = action_batch_t0.float()
    #     obs_batch_t0 = obs_batch_t0.float()
    ...
    def variational_inference_AcT(self, node):
        ...
        # print(f"\nEFE_tau: {EFE_tau}")
        # print(f"type(EFE_tau): {type(EFE_tau)}\n")
    ...
    def path_integrate_AcT(self, node, new_efe_value):
        ...
        while node != None:
            node.visit_count += 1
            node.predictive_EFE += (1 / node.visit_count) * (node.predictive_EFE - new_efe_value)
            # node.predictive_EFE = (1 / node.visit_count) * (node.predictive_EFE - new_efe_value)

            # print(f"\npath_int - node.predictive_EFE: {node.predictive_EFE}\n")
    ...
    def calculate_free_energy_loss(self, obs_batch, action_batch):
        ...
        # avg_log_evidence = self.diag_cov_gaussian_log_evidence(
        #     predicted_cur_obs_mean = generative_observation_mean, 
        #     predicted_cur_obs_var = generative_observation_var, 
        #     observation_samples = reparamed_observation_samples,
        #     D = 4
        ...
        # print(f"\navg_log_evidence: {avg_log_evidence}")
        # print(f"\nneg_log_li: {neg_log_li}")
        # print(f"avg_log_likelihood: {avg_log_likelihood}\n")

        # Compute the final VFE loss
        # loss = kl_divergence - avg_log_likelihood
        # loss = kl_divergence - avg_log_evidence
        loss = kl_divergence - neg_log_li

        return loss, kl_divergence, neg_log_li
        # return loss, kl_divergence, avg_log_likelihood
        # return loss, kl_divergence, avg_log_evidence
    ...
    def learn(self):
        ...
        # free_energy_loss, kl_divergence, avg_log_likelihood = self.calculate_free_energy_loss(obs_batch_t0, action_batch_t0)
        # free_energy_loss, kl_divergence, avg_log_evidence = self.calculate_free_energy_loss(obs_batch_t0, action_batch_t0)
        free_energy_loss, kl_divergence, neg_log_li = self.calculate_free_energy_loss(obs_batch_t0, action_batch_t0)
        ...
        # pred_var_state_batch_avg = torch.mean(pred_var_state_batch_t1, dim = 0)
        # pred_mean_state_batch_avg = torch.mean(pred_mean_state_batch_t1, dim = 0)

        # return free_energy_loss, kl_divergence, avg_log_likelihood # , pred_var_state_batch_avg, pred_mean_state_batch_avg
        # return free_energy_loss, kl_divergence, avg_log_evidence # , pred_var_state_batch_avg, pred_mean_state_batch_avg
        return free_energy_loss, kl_divergence, neg_log_li # , pred_var_state_batch_avg, pred_mean_state_batch_avg
    ...
    def train(self):
        ...
        for ith_episode in range(self.n_episodes):
            ...
            # initial state belief mean and variance
            # state_belief_mean = torch.tensor(noisy_obs, dtype = torch.float32, device = self.device)
            # state_belief_var = torch.tensor([0.01, 0.01, 0.01, 0.01], dtype=torch.float32, device=self.device)
            noisy_obs = torch.tensor(noisy_obs, dtype = torch.float32, device = self.device)
            # state_belief_var = torch.tensor([0.01, 0.01, 0.01, 0.01], dtype=torch.float32, device=self.device)
            ...
            while not done:
                ...
                # predict the next state belief mean and variance:
                # _, state_belief_var = self.variational_transition_net(action.float())

                # increment the per-episode reward:
                total_reward += reward

                # learn, and get the new state belief variance 
                # free_energy_loss, kl_divergence, avg_log_likelihood = self.learn() # return the new state_belief_mean and state_belief_var here ????????????????????????????????????????????????
                # free_energy_loss, kl_divergence, avg_log_evidence = self.learn() # return the new state_belief_mean and state_belief_var here ????????????????????????????????????????????????
                ...
                # print(f"avg_log_likelihood: {avg_log_likelihood}")
                # print(f"avg_log_evidence: {avg_log_evidence}")
    ...
    # def run_trained_networks(self):

    #     # specify the paths for the trained models:
    #     gen_trans_net_path = "networks/intermediary/intermediary_transnet_rX_end.pth"
    #     gen_obs_net_path = "networks/intermediary/intermediary_obsnet_rX_end.pth"
    #     var_trans_net_path = "networks/intermediary/intermediary_vartransnet_rX_end.pth"

    #     # instantiate the rained models:
    #     self.generative_transition_net.load_state_dict(torch.load(gen_trans_net_path, map_location = torch.device('cpu')))
    #     self.generative_observation_net.load_state_dict(torch.load(gen_obs_net_path, map_location = torch.device('cpu')))
    #     self.variational_transition_net.load_state_dict(torch.load(var_trans_net_path, map_location = torch.device('cpu')))

    #     # put the instantiated nets into eval mode:
    #     self.generative_transition_net.eval()
    #     self.generative_observation_net.eval()
    #     self.variational_transition_net.eval()
    ...
    #     obs, _ = self.env.reset()
    #     noisy_obs = obs + noise_std * np.random.randn(*obs.shape)
    #     # noisy_obs = obs + noise_std * np.random.randn(obs[0].shape[0])
    ...
    #                 # pred_mean_cur_state_and_cur_action = torch.cat((noisy_obs.detach(), action.float()), dim = 1)
    #                 pred_mean_cur_state_and_cur_action = torch.cat((noisy_obs.detach(), action.float()), dim = 0)

    #                 state_belief_mean, state_belief_var = self.generative_transition_net(pred_mean_cur_state_and_cur_action)

    #             # take this action in the environment:
    #             obs, reward, truncated, terminated, _ = self.env.step(action[0].item())
    #             done = terminated or truncated
    #             noisy_obs = obs + noise_std * np.random.randn(*obs.shape)
    #             # noisy_obs = obs + noise_std * np.random.randn(obs[0].shape[0])
