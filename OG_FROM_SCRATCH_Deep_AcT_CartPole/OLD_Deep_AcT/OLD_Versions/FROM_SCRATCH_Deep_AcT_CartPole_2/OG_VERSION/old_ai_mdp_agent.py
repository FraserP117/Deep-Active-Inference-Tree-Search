...
class Node:
    ...
    def __init__(self, hidden_state_belief = None, action_at_tau_minus_one = None, parent = None):
        '''
        hidden_state_belief = prediction from transition network
        ...
        self.action_posterior_belief = 0 # this is a distribution over all possible actions?
    ...
    def update_action_posterior_belief(self):
        ...
        if total_visits > 0:
            self.action_posterior_belief = self.visit_count / total_visits
        else:
            self.action_posterior_belief = 0
...
class Agent():
    ...
    def __init__(self, argv):
        ...
        # print(f"\nAgent init - self.obs_shape: {self.obs_shape}")
        # print(f"Agent init - self.obs_size: {self.obs_size}")
        # print(f"Agent init - self.n_actions: {self.n_actions}\n")

        self.freeze_cntr = 0 # Keeps track of when to (un)freeze the target network
        
        # Initialize the networks:
        self.transition_net = Model(self.obs_size+1, self.obs_size, self.n_hidden_trans, lr=self.lr_trans, device=self.device) # OG
        self.policy_net = Model(self.obs_size, self.n_actions, self.n_hidden_pol, lr=self.lr_pol, softmax=True, device=self.device) # OG
        self.value_net = Model(self.obs_size, self.n_actions, self.n_hidden_val, lr=self.lr_val, device=self.device) # OG

        # self.transition_net = Model(int(self.obs_size+1), int(self.obs_size), self.n_hidden_trans, lr=self.lr_trans, device=self.device) # !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
        # self.policy_net = Model(int(self.obs_size), int(self.n_actions), self.n_hidden_pol, lr=self.lr_pol, softmax=True, device=self.device) # !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
        # self.value_net = Model(int(self.obs_size), int(self.n_actions), self.n_hidden_val, lr=self.lr_val, device=self.device) # !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
    ...
    def set_parameters(self, argv):
        ...
        default_parameters = {
            ...
            # 'env':'FrozenLake-v1', 'n_episodes':50,
    ...
    def get_mini_batches(self):
        ...
        # print(f"\nobs_batch_t0: {obs_batch_t0}")
        # print(f"action_batch_t0.float().squeeze(1): {action_batch_t0.float().squeeze(1)}\n")

        # print(f"\nobs_batch_t0.shape: {obs_batch_t0.shape}")
        # print(f"action_batch_t0.float().shape: {action_batch_t0.float().shape}\n")

        # print(f"\nobs_batch_t0.unsqueeze(0).shape: {obs_batch_t0.unsqueeze(0).shape}")
        # print(f"action_batch_t0.float().shape: {action_batch_t0.float().shape}\n")

        # print(f"\nself.transition_net.n_inputs: {self.transition_net.n_inputs}")
        # print(f"self.transition_net.n_hidden: {self.transition_net.n_hidden}")
        # print(f"self.transition_net.n_outputs: {self.transition_net.n_outputs}\n")
        
        # At time t0 predict the state at time t1:
        X = torch.cat((obs_batch_t0, action_batch_t0.float()), dim=1)
        # X = torch.cat((obs_batch_t0.unsqueeze(0), action_batch_t0.float()), dim=0)
    ...
    ################################################### BEGIN Active Inference Tree Search ################################################### 

    def sample_action(self, action_distribution):
    # def select_action_AcT(self, action_distribution):
        actions, probabilities = zip(*action_distribution)
        sampled_action = random.choices(actions, probabilities)[0]
        return sampled_action

    def tree_policy(self, node, B): # use a NEURAL NETOWRK??
        while not self.node_is_terminal_leaf(node):
            if not self.node_fully_expanded(node):
                return self.expand(node, B) # use a NEURAL NETOWRK??
            else:
                node = self.variational_inference(node)
        v_tau = node
        return v_tau

    def expand(self, node, B):
        unused_actions = self.get_unused_actions(node)
        a_prime = random.choice(unused_actions)
        x_prime = B(a_prime) * node.state
        child = Node(hidden_state_belief=x_prime, action=a_prime, parent=node)
        node.children.append(child)
        return child

    def variational_inference(self, node):
        exploration_factor = math.sqrt(2 * math.log(node.parent.visit_count) / node.visit_count)
        # probabilities = [math.exp(exploration_factor * child.quality) for child in node.parent.children] # OG
        ...
        return self.find_child_node(node, a_prime)

    def evaluate(self, node, A, B, C, delta):
        g = self.expected_free_energy(node.state, node.observation, node.action, A, B, C) # NEURAL NETWORK APPROXIMATION
        g_delta = delta ** node.visit_count * g
        return g_delta

    def path_integration(self, node, g_delta):
        while node != None:
            node.visit_count += 1
            # node.quality += (1 / node.visit_count) * (g_delta - node.quality) # OG
            node.predictive_EFE_tau += (1 / node.visit_count) * (g_delta - node.predictive_EFE_tau)
            node = node.parent

    # Helper functions

    def halting_conditions_satisfied(self, t):
        # simply test to see if within alllowable runtime - can make more sophisticated
        return t < 5

    def node_is_terminal_leaf(self, node):
    ...
    def get_unused_actions(self, node):
        # Implement your own logic to retrieve the unused actions from the node
        return []

    def find_child_node(self, node, action):
        for child in node.children:
            if child.action == action:
                return child

    def update_expected_state_belief(self):
        # Implement your own function to update the expected state belief
        return x_t

    def expected_free_energy(self, state, observation, action, A, B, C):
    ...
    def active_inferece_tree_search(self, initial_hidden_state_belief, delta, epsilon):
        '''
        ...
        root_node = Node(
            hidden_state_belief = initial_hidden_state_belief, action_at_tau_minus_one = None, parent = None
        )

        # Perform Active Inference Tree Search iteratively
        t = 0

        while not self.halting_conditions_satisfied(t):

            # update expected state belief x_{t-1} using s_t, o_t, a_{t-1}, A, B, D
            x_t = self.update_expected_state_belief() # NEURAL NETWORK APPROXIMATION
            ...
            while delta ** root_node.visit_count < epsilon:
                ...
                selected_node = self.tree_policy(root_node, B)
                ...
                # Stage 3: Evaluation
                value = self.evaluate(root_node, A, B, C, delta)

                # Stage 4: Path Integration
                self.path_integration(root_node, value)

            # # Update time step and other variables - do I even need this?
            # s_t_plus_1, o_t_plus_1, x_t, a_t = self.extract_information(root_node)
        ...
        '''
        DOES AcT RETURN A BOLTZMANN DISTRIBUTION OVER CONTROL STATES: P(u), FROM WHICH THE OPTIMAL 
        ACTION: a_t CAN BE SAMPLED.
        '''

        # FAIRLY SURE THAT WE SHOULD RETURN THE ROOT NODE
        return root_node
    ...
    ################################################### END Active Inference Tree Search ###################################################

    def select_action(self, obs):
        '''
        THIS WILL BE HANDLED BY AcT EVENTUALLY
        '''
        with torch.no_grad():
            # print(f"\nselect_action - obs: {obs}")
            # print(f"select_action - type(obs): {type(obs)}")
            # print(f"select_action - obs.shape: {obs.shape}\n")
    ...
    def train(self):
        ...
        for ith_episode in range(self.n_episodes):
            ...
            # obs = torch.unsqueeze(torch.tensor(obs, dtype=torch.float32, device=self.device), dim=0).unsqueeze(0)

            # print(f"\ntrain - obs: {obs}")
            # print(f"train - type(obs): {type(obs)}")
            # print(f"train - obs.shape: {obs.shape}\n")
            ...
            while not done:
                ...
                # PROVIDE initial_hidden_state_belief, delta, epsilon
                # initial_hidden_state_belief afforded by the transition network?

                # DO ACTIVE INFERENCE TREE SEARCH
                # root_node = self.active_inferece_tree_search(initial_hidden_state_belief, delta, epsilon)

                # # Select the best action based on the updated tree
                # best_action = self.select_action_AcT(root_node.action_posterior_belief) - self.select_action(obs)

                # # Execute the selected action and transition to a new state
                # transition(best_action) - self.env.step
...
#### OG CODE PRIOR TO INTEGRATION WITH AcT ####
...
#     ################ PASTED AcT TEMPLATE CODE BELOW ################ 
...
#         while not self.halting_conditions_satisfied():
...
#     ################ PASTED AcT TEMPLATE CODE ABOVE ################ 
...
#             '''
#             MIGHT NEED ANOTHER BATCH DIMENSION
#             '''
