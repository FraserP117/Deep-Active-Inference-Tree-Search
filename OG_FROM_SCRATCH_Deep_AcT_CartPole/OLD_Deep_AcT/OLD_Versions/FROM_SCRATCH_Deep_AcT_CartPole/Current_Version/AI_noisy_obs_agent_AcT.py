...
class MVGaussianModel(nn.Module):
    ...
    def __init__(self, n_inputs, n_outputs, n_hidden, lr = 1e-4, device = 'cuda:0', model = None):
    ...
    #     log_var = torch.log(self.stdev(x_2))  # Output log of the diagonal variances

    #     if self.model == 'var_trans':
    #         print(f"\nx: {x}")
    #         print(f"x_1: {x_1}")
    #         print(f"x_2: {x_2}")
    #         print(f"{self.model}_mean: {mean}")
    #         print(f"{self.model}_log_var: {log_var}\n")
...
class Agent():
    ...
    def calculate_approximate_EFE(self, mean_state_phi, var_state_phi, mean_obs_xi, var_obs_xi):
        ...
        with torch.no_grad():
            ...
            # print(f"expected_free_energy: {expected_free_energy}\n")
    ...
    def train(self):
        ...
        for ith_episode in range(500):
            ...
            while not done:
                ...
                avg_avg_ll += avg_log_likelihood
                # free_energy_loss = self.learn()
            ...
            avg_avg_avg = avg_avg_ll / num_loop_iters
            avg_avg_lls.append(avg_avg_avg)

            # loss.cpu().detach().numpy()
            print(f"avg_avg_avg: {avg_avg_avg}")
            ...
            if ith_episode > 0 and ith_episode % self.print_timer == 0:
                ...
                msg = "Episodes: {:4d}, avg score: {:3.2f}, over last {:d}: {:3.2f}, free_energy_loss: {:3.2f}".format(ith_episode, avg_reward, self.print_timer, last_x, free_energy_loss)
                print(msg)
        ...
        def animate(avg_avg_lls):
            plt.cla()
            plt.plot(avg_avg_lls, label = 'Avg of Avg Log Likelihood (per episode)')
            plt.show()
