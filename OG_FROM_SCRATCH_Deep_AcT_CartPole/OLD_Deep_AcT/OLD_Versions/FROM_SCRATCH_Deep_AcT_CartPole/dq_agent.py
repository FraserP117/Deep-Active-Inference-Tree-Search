...
class ReplayMemory():
    ...
    def __init__(self, capacity, obs_shape, device='cpu'):
        ...
        # self.done_mem = torch.empty(capacity, dtype=torch.int8, device=self.device) # OG CODE
    ...
    # def push(self, obs, action, reward, done): # OG CODE
    def push(self, obs, action, reward, terminated, truncated):
        ...
        # self.done_mem[self.position()] = done # OG CODE
    ...
    # def sample(self, obs_indices, action_indices, reward_indices, done_indices, max_n_indices, batch_size): # OG CODE
        ...
        for i in range(len(end_indices)):
            ...
            for j in range(1, max_n_indices):
                if self.terminated_mem[index-j] or self.truncated_mem[index-j]: # if self.done_mem[index-j]: # OG CODE
                    ...
                    # for k in range(len(done_indices)): # OG CODE
                    #     if done_indices[k] >= j: # OG CODE
                    #         done_batch[i, k] = torch.zeros_like(self.done_mem[0]) # OG CODE
...
class Agent():
    ...
    def __init__(self, argv):
        ...
        # self.done_indices = [0] # OG CODE
        self.terminated_indicies = [0]
        self.truncated_indicies = [0]
        # self.max_n_indices = max(max(self.obs_indices, self.action_indices, self.reward_indices, self.done_indices)) + 1 # OG CODE
    ...
    def learn(self):
        ...
        #         self.obs_indices, self.action_indices, self.reward_indices, self.done_indices, self.max_n_indices, self.batch_size) # OG CODE

        # done_batch
        all_obs_batch, action_batch, reward_batch, terminated_batch, truncated_batch = self.memory.sample(
        ...
        # target_batch = reward_batch + (1-done_batch) * self.gamma * target_out.max(1)[0].view(self.batch_size, 1) # OG CODE

        # print(f"\n\nterminated_batch:\n{terminated_batch}")
        # print(f"truncated_batch:\n{truncated_batch}\n\n")

        # print(f"\n\n(1 - terminated_batch):\n{(1 - terminated_batch)}")
        # print(f"(1 - truncated_batch):\n{(1 - truncated_batch)}\n\n")

        # print(f"\n\ntype(terminated_batch): {type(terminated_batch)}")
        # print(f"type(truncated_batch): {type(truncated_batch)}\n\n")

        # done = truncated_batch | terminated_batch

        # print(f"\n\ntruncated_batch | terminated_batch: {done}\n\n")


        target_batch = reward_batch + (1 - (truncated_batch | terminated_batch)) * self.gamma * target_out.max(1)[0].view(self.batch_size, 1)
    ...
    def train(self):
        ...
        for ith_episode in range(self.n_episodes):
            ...
            while not done:
                ...
                # self.memory.push(obs, action, reward, done) # OG CODE
                self.memory.push(obs, action, reward, terminated, truncated) # push(obs, action, reward, terminated, truncated)

                # # OG CODE
                # obs, reward, done, _ = self.env.step(action[0].item())
                obs, reward, terminated, truncated, info = self.env.step(action[0].item())
                ...
                if terminated or truncated:
                    ...
                    # self.memory.push(obs, -99, -99, done) # probably won't get here... # OG CODE
                    self.memory.push(obs, -99, -99, terminated, truncated) # probably won't get here...
                    # observation, info = env.reset()
