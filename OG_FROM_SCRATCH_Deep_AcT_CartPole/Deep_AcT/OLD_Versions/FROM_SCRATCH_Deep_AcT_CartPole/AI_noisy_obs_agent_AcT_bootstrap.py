...
class MVGaussianModel(nn.Module):
    ...
    def forward(self, x):
        ...
        mean = self.mean_fc(x)
        log_var = torch.log(self.var_fc(x))  # Output log of the diagonal variances
    ...
    def reparameterize(self, mean, log_var): # this should be called in get_mini_batches_AcT and self.forward - as per H and L
        '''
        pass the input to the "decoder" model through here first and then 
        feed the output of this to the "decoder" model. 
        '''

        std = torch.exp(0.5 * log_var)  # Exponentiate to get the standard deviations
...
class Node:
    ...
    def __init__(self, pred_mean_state = None, action_at_tau_minus_one = None, parent = None):
        ...
        self.pred_mean_state = pred_mean_state # MAP state-estimate: mean of the state belief.
...
class Agent():
    ...
    def diagonal_gaussian_entropy(self, sigma_squared, D):
        ...
        log_det_sigma = torch.log(torch.prod(sigma_squared))
    ...
    #     '''
    #     Returns an upper bound aproximation of the Expected Free Energy - formualted as hidden state belief divergence.
    ...
    def get_mini_batches_AcT(self):
        ...
        pred_mean_state_batch_t1, pred_var_state_batch_t1_var = self.variational_transition_net(action_batch_t0) # might need to normalise the inputs
        pred_var_state_batch_t1 = torch.exp(pred_var_state_batch_t1_var) # REPARAMETERIZE HERE OR NOT???????????????????????????
        # torch.clamp(pred_mean_state_batch_t1, min = -4.8, max = 4.8)
        # torch.clamp(pred_var_state_batch_t1, min = -3.0, max = 3.0)
    ...
    def expand_AcT(self, node):
        ...
        # torch.clamp(mean_next_state_phi, min = -4.8, max = 4.8)
        # torch.clamp(var_next_state_phi, min = -3.0, max = 3.0)
        ...
        # torch.clamp(mean_next_obs_xi, min = -4.8, max = 4.8)
        # torch.clamp(var_next_obs_xi, min = -3.0, max = 3.0)

        # instantiate a child node as a consequence of performing action a_prime
        child_node = Node()
        child_node.parent = node
        child_node.depth = node.depth + 1
        # child_node.pred_mean_state = (mean_next_state_phi, var_next_state_phi) # x_tau = Q(s_t | pi)
        ...
        raw_efe = self.calculate_approximate_EFE(mean_next_state_phi, var_next_state_phi, mean_next_obs_xi, var_next_obs_xi) # WRONG! SHOULD BE W.R.T VARIATIONAL MODEL!!!!!!
    ...
    def calculate_free_energy_loss(self, pred_mean_state_batch_t1, action_batch_t1):
        ...
        generative_cur_observation_distribution_mean, generative_cur_observation_distribution_log_var = self.generative_observation_net(pred_mean_cur_state) # predicted current obs (gen)
    ...
    #     # print(f"\n\ntorch.mean(generative_next_state_distribution_mean, dim = 0):\n{torch.mean(generative_next_state_distribution_mean, dim = 0)}\ntorch.mean(generative_next_state_distribution_var, dim = 0):\n{torch.mean(generative_next_state_distribution_var, dim = 0)}\n")
    #     # print(f"torch.mean(variational_next_state_distribution_mean, dim = 0):\n{torch.mean(variational_next_state_distribution_mean, dim = 0)}\ntorch.mean(variational_next_state_distribution_var, dim = 0):\n{torch.mean(variational_next_state_distribution_var, dim = 0)}\n")
    #     # print(f"torch.mean(generative_cur_observation_distribution_mean, dim = 0):\n{torch.mean(generative_cur_observation_distribution_mean, dim = 0)}\ntorch.mean(generative_cur_observation_distribution_var, dim = 0):\n{torch.mean(generative_cur_observation_distribution_var, dim = 0)}\n\n")
    ...
    #     # Use the reparameterization trick to sample from the distribution
    #     reparamed_hidden_state_samples = self.generative_transition_net.reparameterize(generative_next_state_distribution_mean, generative_next_state_distribution_log_var) # keept as logvar 

    #     # # Calculate the negative log likelihood of the observation
    #     # neg_log_likelihood = self.diag_cov_gaussian_log_li(
    #     #     predicted_cur_obs_mean = generative_cur_observation_distribution_mean, 
    #     #     predicted_cur_obs_var = generative_cur_observation_distribution_var, 
    #     #     input_reparam_sampled_hidden_state = generative_next_state_distribution_mean,
    #     #     D = 4
    #     # )
    ...
    #         predicted_cur_obs_mean = generative_cur_observation_distribution_mean, 
    #         predicted_cur_obs_var = generative_cur_observation_distribution_var, 
    #         input_reparam_sampled_hidden_state = reparamed_hidden_state_samples, # input reparam "observation" or "state"?
    #         D = 4
    #     )
...
#     agent.train()
 ...
 # def calculate_approximate_EFE(self, mean_state_theta, var_state_theta, mean_obs_xi, var_obs_xi):
    ...
    # def diag_cov_gaussian_log_li(self, predicted_cur_obs_mean, predicted_cur_obs_var, input_reparam_sampled_hidden_state, D):

    #     # sum_term = torch.sum(
    #     #     torch.log(predicted_cur_obs_var) + ((input_reparam_sampled_hidden_state - predicted_cur_obs_mean) ** 2)/(predicted_cur_obs_var)
    #     # )

    #     # nll = 0.5 * (D * np.log(2 * np.pi) + sum_term)

    #     print(f"\npredicted_cur_obs_var.shape: {predicted_cur_obs_var.shape}")
    #     print(f"input_reparam_sampled_hidden_state.shape: {input_reparam_sampled_hidden_state.shape}")
