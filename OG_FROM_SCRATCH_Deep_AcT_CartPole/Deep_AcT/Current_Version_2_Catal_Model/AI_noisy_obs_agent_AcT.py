...
class Agent():
    ...
    def set_parameters(self, argv):
        ...
        default_parameters = {
            ...
            'env':'CartPole-v1', 'n_episodes':300, # OG VERSION FOR CARTPOLE-V1
    ...
    def expand_AcT(self, node):
        ...
        # get the node's currrent state:
        current_state_pred_mean = node.pred_mean_state

        # predict the corresponding observation:
        mean_next_obs_xi, var_next_obs_xi = self.generative_observation_net(current_state_pred_mean)

        # print(f"\n current_state_pred_mean.shape: {current_state_pred_mean.shape}")
        # print(f"a_prime.shape: {a_prime.shape}")
        # print(f"mean_next_obs_xi.shape: {mean_next_obs_xi.shape}\n")

        # concatenate the chosen action, current state and observation:
        var_trans_net_arg = torch.cat((current_state_pred_mean, a_prime, mean_next_obs_xi), dim = 0) # AS PER CATAL ET AL

        # print(f"var_trans_net_arg: {var_trans_net_arg}\n")
        # breakpoint()
        ...
        mean_next_state_phi, var_next_state_phi = self.variational_transition_net(var_trans_net_arg) # action_batch_t1.float()
    ...
    def calculate_free_energy_loss(self, obs_batch, action_batch):
        ...
        # print(f"\ngenerative_next_state_distribution_mean.shape: {generative_next_state_distribution_mean.shape}")
        # print(f"current_action.shape: {current_action.shape}")
        # print(f"obs_batch.shape: {obs_batch.shape}\n")

        var_trans_net_arg = torch.cat((generative_next_state_distribution_mean, current_action, obs_batch), dim = 1) # AS PER CATAL ET AL

        # print(f"var_trans_net_arg: {var_trans_net_arg}\n")
        # breakpoint()

        # variational_next_state_distribution_mean, variational_next_state_distribution_var = self.variational_transition_net(current_action) # predicted next state prob (var)
        variational_next_state_distribution_mean, variational_next_state_distribution_var = self.variational_transition_net(var_trans_net_arg) # predicted next state prob (var)
    ...
    #     self.generative_transition_net.load_state_dict(torch.load(gen_trans_net_path))
    #     self.generative_observation_net.load_state_dict(torch.load(gen_obs_net_path))
    #     self.variational_transition_net.load_state_dict(torch.load(var_trans_net_path))
